...
Răspunde DOAR cu JSON valid de forma: {"insights":[{"insight_text":"...","action":"..."|null}]}`;
    ...
    // Optimizarea reclamelor merge pe Opus (mai deștept) prin stratul central.
    const { askClaude, LLM } = await import("@/lib/llm.server");
    const raw = await askClaude("Răspunzi DOAR cu JSON valid, fără markdown.", prompt, {
      model: LLM.reasoning,
    });
