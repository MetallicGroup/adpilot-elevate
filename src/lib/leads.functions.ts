...
/**
 * Manual "Refresh": pulls Meta Lead Ads leads for the current user.
 * Uses the same incremental engine as the cron job; pass `full: true` to
 * ignore the stored watermark and rescan every form from the beginning.
 */
export const syncMetaLeads = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) => z.object({ full: z.boolean().default(false) }).parse(d ?? {}))
  .handler(async ({ data, context }) => {
    const { syncMetaLeadsForUser } = await import("@/lib/leads-sync.server");
    return syncMetaLeadsForUser(context.userId, { full: data.full });
  });